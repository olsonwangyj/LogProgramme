...
Batch mode scans a DHR folder and creates one output set per valid iSRT case folder:

```powershell
python -m app.log_activity_tool ^
  --dhr-root "D:\LogProgramme\DHR" ^
  --batch-output "D:\LogProgramme\DHR_output" ^
  --distribution-image-gallery ^
  --no-file-picker
```

Batch mode groups `Biobot\System*\Log` folders back to the nearest case folder, for example `DHR\iSRT300027\C` becomes `iSRT300027_C` and `DHR\iSRT300040\23Apr\C` becomes `iSRT300040_23Apr_C`. For each case, it checks `UroBiopsy_*.txt` files newest first and selects the newest file with at least `--min-initialization-count` completed initialization cycles. The initialization count is the number of `@robot initialization done` lines in the TXT file. If no TXT reaches the threshold, that case is skipped and the reason is recorded in `cross_iSRT_summary.xlsx`.
...
The summary workbook contains only `Overall Axis Action Summary` with `Axis`, `Action`, `n`, `Mean (s)`, `SD (s)`, `Var (s²)`, `Median (s)`, `IQR (s)`, `Min–Max (s)`, and `CV (%)`. `Search Reference` is intentionally excluded from this standalone summary report only. `Move to Home` remains included. Gallery sheets, image statistics, chart images, raw data, debug metadata, full paths, and detailed records are not included.
...
Batch output uses one folder per case and writes a cross-case summary workbook at the output root:

```text
DHR_output/
  iSRT300027_C/
    iSRT300027_C.xlsx
    iSRT300027_C_summary.xlsx
    iSRT300027_C_distribution_image_gallery.xlsx
    iSRT300027_C_distribution_charts/
  iSRT300040_23Apr_C/
    ...
  cross_iSRT_summary.xlsx
```

`cross_iSRT_summary.xlsx` uses the same first-sheet format as an individual summary workbook. Its first sheet is `Overall Axis Action Summary`, grouped by `Axis` and `Action`, with statistics calculated from pooled raw duration samples across all processed iSRT cases. It does not average per-case means. `Search Reference` is excluded there too and `Move to Home` remains included. Audit sheets follow the main summary: `Case Processing Summary` and `Case Contribution Details`.
...
`Axis Action Summary` is the compact table for quick review. The standalone summary workbook contains one sheet named `Overall Axis Action Summary` and uses the final user-facing labels:

```text
Axis | Action | n | Mean (s) | SD (s) | Var (s²) | Median (s) | IQR (s) | Min–Max (s) | CV (%)
```

Rows include hardware-distance motion actions such as `Clear Motor`, `Move to Home`, and `Move to Max`. `Search Reference` is excluded from the standalone summary workbook only; it remains available in `Reference Duration Summary`, `Reference Duration Raw Data`, charts, gallery/statistics outputs, and detailed audit sheets. Motion rows use the same validated hardware-overlap rows used by `Distribution Summary`, so mean, SD, variance, median, IQR, min/max, and CV remain consistent with the detailed statistics.
